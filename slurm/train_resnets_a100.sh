#!/bin/bash
# Same content as train_resnets_a100.sbatch but with .sh extension for sites
...
#   sbatch --array=0-4 slurm/train_resnets_a100.sh
...
#SBATCH --output=slurm_logs/resnet_%A_%a.out
#SBATCH --error=slurm_logs/resnet_%A_%a.err
# NOTE: this script is intended to be submitted as an array job, e.g.
#   sbatch --array=0-4 slurm/train_resnets_a100.sh
# or use the provided helper `slurm/submit_resnets.sh` which computes the
...
# -----------------------------------------------------------------------------
CONFIGS=(
"--arch resnet32 --lr 0.1  --batch-size 128 --epochs 60 --save-dir results/resnet32"
"--arch resnet44 --lr 0.1  --batch-size 128 --epochs 60 --save-dir results/resnet44"
)
