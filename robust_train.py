...
parser = argparse.ArgumentParser(description='Adversarial Training for CIFAR10 in pytorch')
...
parser.add_argument('--pgd-steps', default=7, type=int,
                    help='number of PGD steps for adversarial perturbation (default: 7)')
parser.add_argument('--pgd-step-size', default=2/255, type=float,
                    help='step size for PGD attack (default: 2/255)')
best_prec1 = 0
...
def main():
    ...
    # Create normalization wrapper for adversarial training
    # This applies normalization inside the perturbation computation
    # normalize_tensor = lambda x: normalize(x)

    # For CPU, reduce number of workers to prevent issues
    num_workers = 0 if device.type == 'cpu' else args.workers
    ...
    print(f"  PGD step size: {args.pgd_step_size:.6f}")
...
def train(train_loader, model, criterion, optimizer, epoch, device):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # Concatenate clean and adversarial examples for a batch
        input_combined = torch.cat([input_var, input_adv], dim=0)
        target_combined = torch.cat([target_var, target_var], dim=0)
...
def validate(val_loader, model, criterion, device):
    """
    Run evaluation on clean examples
    """
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
